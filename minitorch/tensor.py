...
def ones(shape):
  return Tensor.make([1.0] * int(operators.prod(shape)), shape)


def rand(shape):
  """
  Produce a random tensor of size `shape`.
  Args: shape (tuple): shape of tensor.
  Returns: New tensor
  """
...
def make_tensor_functions(backend):
  ...
  class TF:
    ...
    class Sigmoid(Function):
      ...
      def backward(ctx, grad_output):
        sm = ctx.saved_values
        m1 = mul_zip(sm, add_zip(tensor_fromlist([1]), -sm))
        return mul_zip(m1, grad_output)
    ...
    class ReLU(Function):
      ...
      @staticmethod
      def backward(ctx, grad_output):
        return relu_back_zip(ctx.saved_values, grad_output)
    ...
    class Log(Function):
      ...
      @staticmethod
      def backward(ctx, grad_output):
        return log_back_zip(ctx.saved_values, grad_output)

    class Exp(Function):
      @staticmethod
      def forward(ctx, a):
        em = exp_map(a)
        ctx.save_for_backward(em)
        return em

      @staticmethod
      def backward(ctx, grad_output):
        em = ctx.saved_values
        return mul_zip(em, grad_output)
    ...
    class Sum(Function):
      ...
      def forward(ctx, a, dim):
        ...
        if dim is not None:
          ...
          dims = list(range(a.dims))
          return add_reduce(a, dims).view(1)

      @staticmethod
      def backward(ctx, grad_output):
        sh = ctx.saved_values
        return mul_zip(ones(sh), grad_output)
    ...
    class Mean(Function):
      ...
      def forward(ctx, a, dim):
        if dim is not None:
          N = a.shape[dim]
          mean_ = add_reduce(a, [dim])
          mean_ = mul_zip(mean_, ones(mean_.shape) / a.shape[dim])
        else:
          N = tensor_fromlist([np.prod(a.shape)])
          mean_ = add_reduce(a, list(range(a.dims))).view(1)
          mean_ = mean_ / N
        ctx.save_for_backward(a.shape, N)
        return mean_

      @staticmethod
      def backward(ctx, grad_output):
        a_shape, N = ctx.saved_values
        return mul_zip(ones(a_shape) / N, grad_output)

    class LT(Function):
      @staticmethod
      def forward(ctx, a, b):
        lt = lt_zip(a, b)
        ctx.save_for_backward(lt.shape)
        return lt

      @staticmethod
      def backward(ctx, grad_output):
        return zeros(ctx.saved_values)

    class EQ(Function):
      @staticmethod
      def forward(ctx, a, b):
        eqs = eq_zip(a, b)
        ctx.save_for_backward(eqs)
        return eqs

      @staticmethod
      def backward(ctx, grad_output):
        return mul_zip(grad_output, ctx.saved_values)
...
def grad_check(f, *vals):
  ...
  for i, x in enumerate(vals):
    ...
    np.testing.assert_allclose(g[ind], check, 1e-2, 1e-2)
